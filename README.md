...
# react-native-camera-kit-spatacus

### 兼容rn 0.60及以上版本
### 其他可以参考react-native-camera-kit
...
```bash
npm install react-native-camera-kit-spatacus --save
```

Or if you're using yarn:

```bash
yarn add react-native-camera-kit-spatacus
```

#### iOS

- Locate the module lib folder in your node modules: `PROJECT_DIR/node_modules/react-native-camera-kit-spatacus/ios/lib`
...
```diff
+ include ':rncamerakit'
+ project(':rncamerakit').projectDir = new File(rootProject.projectDir, '../node_modules/react-native-camera-kit-spatacus/android/')
```
